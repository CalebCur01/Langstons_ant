...
#This is 3 state 2 color "snowflake" turmite


ROW_COUNT = 100
COLUMN_COUNT = 100

SCREEN_TITLE = "Snowflake"
...
rules = ((1,8,1),(1,2,0),(1,4,1),(1,4,2),(),(0,4,0))
        ...
        for row in range(ROW_COUNT):                    #[][][]
            for column in range(COLUMN_COUNT):          #[][][]
                ...
                if grid[row][column] == 0:
                    ...
                    self.spritelist[location].alpha = 0
...
def update_state():
    ...
    if(not color):
        ...
        if(cur_state == 1):
            cur_rule = rules[2] #{1,4,1}
        if(cur_state == 2):
            cur_rule = rules[4] #{} blank ruleset (should never reach this)
    if(color):
        if(cur_state == 0):
            cur_rule = rules[1] #{1,2,0}
        if(cur_state == 1):
            cur_rule = rules[3] #{1,4,2}
        if(cur_state == 2):
            cur_rule = rules[5] #{0,4,0}
